...
import { useStoreSubscription, usingSharedKey, type Config } from "@/lib/store";
...
export function ConfigureScreen({ cfg, setCfg, stagedFiles, onBack, onStart, isRtl, starting = false }: Props) {
  // Re-render when /api/config arrives so the canStart gate flips correctly
  // once we learn whether the server has a shared key.
  ...
  // The shared/personal choice lives entirely in the settings modal. Here we
  // just respect the resulting cfg: if the user picked shared mode, the
  // server covers the API key, so don't require one in the form.
  const needsKey = !usingSharedKey(cfg) && cfg.providerId !== "ollama";
  const canStart =
    !!cfg.target &&
    !!cfg.shape &&
    !!cfg.model.trim() &&
